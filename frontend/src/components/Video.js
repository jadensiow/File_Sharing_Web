...
import { addViews } from "../redux/action/channelActions";
...
const Video = ({ video }) => {
  ...
  const videoSelector = useSelector((state) => state.watchVideo.video);
  ...
  useEffect(() => {
    dispatch(addViews(token, videoSelector));
  }, []);
    ...
    <div className="my-5">
      ...
      <Row>
        <Col xs={2}>
          ...
          <Row>
            <Link to={`/${video.user.id}/channel`} className="react-link">
